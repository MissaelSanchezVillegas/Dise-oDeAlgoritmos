...
class Vertex:
    ...
    def add_neighbor(self, _neighbor, _weight=1):
        if _neighbor not in self.neighbors.keys():
            ...
            if type(self.neighbors[_neighbor]) is list:
                if type(_weight) is list:
                    self.neighbors[_neighbor].extend(_weight)
                else: 
                    self.neighbors[_neighbor].append(_weight)
            else:
                if type(_weight) is list:
                    _weight.append(self.neighbors[_neighbor])
                    self.neighbors[_neighbor] = _weight
                else: 
                    self.neighbors[_neighbor] = [self.neighbors[_neighbor], _weight]
